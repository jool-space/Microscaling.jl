...
x_scale = relayout(Sm1xx(), CuArray(zeros(Scale, K_s, M)));

y = CuArray(randn(Element, K_s * SCALE_BLOCK_SIZE, N));
y_scale = relayout(Sm1xx(), CuArray(zeros(Scale, K_s, N)));
...
bench = @be CUDACore.@sync @cuda backend=ct blocks=(cld(M, TM), cld(N, TN)) mxfp8_gemm(
    X.p,
